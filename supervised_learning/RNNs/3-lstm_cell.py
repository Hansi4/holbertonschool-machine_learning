...
class LSTMCell:
    """ A python function that represents an LSTM unit """
    ...
    def forward(self, h_prev, c_prev, x_t):
        ...
        def softmax(x):
            """ A python function that compute softmax activation function """
            return np.exp(x) / np.sum(np.exp(x), axis=1, keepdims=True)

        def sigmoid(x):
            """ A python function that compute sigmoid activation function"""
            return 1 / (1 + np.exp(-x))
